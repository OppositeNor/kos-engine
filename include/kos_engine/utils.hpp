...
public:
     ...
     */
    static std::vector<CGVisualImage*> CreateVisualImageList(const std::initializer_list<KEString>& p_image_rks)
    {
        std::vector<CGVisualImage*> list;
        for (auto iter : p_image_rks)
        {
            list.insert(list.end(), CGCreateVisualImage(iter.c_str(), KEGame::GetInstance()->GetGameWindow(), CG_FALSE));
        }
        return list;
    }
